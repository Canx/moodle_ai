...
class BrowserManager:
    """
    Gestiona un browser Playwright singleton para evitar procesos huérfanos.
    Incluye cleanup automático con signal handlers y atexit.
    """
    _instance = None
    ...
    async def get_browser(cls) -> Browser:
        """Obtiene el browser singleton, creándolo si no existe."""
    ...
    async def get_page(cls) -> Page:
        """Crea una nueva página en el browser singleton."""
    ...
    async def _initialize(cls):
        ...
        if cls._browser is None or not cls._browser.is_connected():
            logger.info("BrowserManager: Iniciando browser Chromium...")
            cls._browser = await cls._playwright.chromium.launch(
                headless=True,
                args=["--no-sandbox", "--disable-dev-shm-usage"]
            )
            logger.info("BrowserManager: Browser iniciado correctamente")

        # Registrar cleanup handlers una sola vez
    ...
    def _register_cleanup_handlers(cls):
        """Registra handlers para cleanup en señales y atexit."""
        def sync_cleanup(signum=None, frame=None):
            """Cleanup síncrono para signal handlers."""
            logger.info(f"BrowserManager: Recibida señal de terminación, cerrando browser...")
            try:
                ...
                logger.error(f"BrowserManager: Error en cleanup síncrono: {e}")
                # Forzar cierre si falla el cleanup normal
                cls._force_cleanup()
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            try:
                ...
                logger.warning(f"BrowserManager: No se pudo registrar handler para {sig}: {e}")

        # Atexit como fallback
        atexit.register(cls._force_cleanup)
        logger.info("BrowserManager: Cleanup handlers registrados")

    @classmethod
    def _force_cleanup(cls):
        """Cleanup forzado síncrono para atexit."""
        if cls._browser:
            try:
                # Intentar matar los procesos del browser directamente
                import subprocess
                if hasattr(cls._browser, '_impl_obj') and hasattr(cls._browser._impl_obj, '_browser'):
                    pid = getattr(cls._browser._impl_obj._browser, 'pid', None)
                    if pid:
                        logger.info(f"BrowserManager: Matando proceso browser PID {pid}")
                        subprocess.run(['kill', '-9', str(pid)], capture_output=True)
            except Exception as e:
                logger.error(f"BrowserManager: Error en force_cleanup: {e}")

    @classmethod
    async def cleanup(cls):
        """Cierra el browser y Playwright de forma limpia."""
        async with cls._lock:
            if cls._browser:
                try:
                    logger.info("BrowserManager: Cerrando browser...")
                    await cls._browser.close()
                    logger.info("BrowserManager: Browser cerrado")
                except Exception as e:
                    logger.error(f"BrowserManager: Error cerrando browser: {e}")
                finally:
                    cls._browser = None

            if cls._playwright:
                try:
                    logger.info("BrowserManager: Deteniendo Playwright...")
                    await cls._playwright.stop()
                    logger.info("BrowserManager: Playwright detenido")
                except Exception as e:
                    logger.error(f"BrowserManager: Error deteniendo Playwright: {e}")
                finally:
...
def search_courses(query: str, moodle_alias: str = "default") -> Dict[str, Any]:
    """
    Busca cursos por nombre usando coincidencia difusa (ignora acentos, mayúsculas y tolera pequeños errores).
    """
    import unicodedata
    import difflib
    ...
    found = []
    for c in cursos:
        name_norm = normalize(c.get("nombre", ""))
        name_terms = name_norm.split()
        
        # Verificar que CADA término de la query coincida "algo" con ALGÚN término del nombre
        all_terms_match = True
        for q_term in query_terms:
            # Buscar si q_term se parece a alguna palabra del nombre
            match_found = False
            for n_term in name_terms:
                if q_term in n_term: # Substring match (ej: "bio" encaja en "biologia")
                    match_found = True
                    break
                # Fuzzy match para errores (ej: "inteligencia" vs "intelligencia")
                ratio = difflib.SequenceMatcher(None, q_term, n_term).ratio()
                if ratio > 0.85: # Umbral de similitud
                    match_found = True
                    break
    ...
    return {
        ...
        "cursos": found,
        "info": f"Búsqueda sobre datos cacheados ({last})"
    }
...
# Timeout para subprocesos externos (libreoffice, pdftoppm, etc)
SUBPROCESS_TIMEOUT = 60  # segundos


async def run_subprocess_with_timeout(cmd: str, timeout: int = SUBPROCESS_TIMEOUT) -> tuple:
    """Ejecuta un subproceso con timeout para evitar procesos colgados."""
    ...
    try:
        ...
        logger.warning(f"Subproceso timeout después de {timeout}s: {cmd[:50]}...")
        proc.kill()
        await proc.wait()
...
async def analyze_submission_file(file_url: str, moodle_alias: str = "default") -> Dict[str, Any]:
    ...
    # Extraer IDs de la URL para organizar carpetas
    # URL típica: .../pluginfile.php/CONTEXTO/assignsubmission_file/submission_files/SUBMISSION_ID/filename
    import re
    submission_id = "unknown_submission"
    match = re.search(r"/submission_files/(\d+)/", file_url)
    if match:
        submission_id = match.group(1)

    # Estructura: tmp/moodle_downloads/submission_ID/
    save_dir = os.path.join(tmp_dir, submission_id)
    os.makedirs(save_dir, exist_ok=True)

    # Usar BrowserManager pero crear un context especial para descargas
    ...
    try:
        ...
        # Guardar con nombre original en carpeta específica
        file_path = os.path.join(save_dir, filename)

        # Si ya existe, añadir un sufijo para no sobrescribir si hay varios archivos con mismo nombre en misma entrega
        if os.path.exists(file_path):
            file_path = os.path.join(save_dir, f"{uuid.uuid4()}_{filename}")
        ...
        if ext in [".docx", ".odt", ".doc"]:
            try:
                # Usar LibreOffice para convertir a HTML
                # Output dir debe ser específico para la conversión
                out_dir = os.path.join(save_dir, f"convert_{uuid.uuid4()}")
                os.makedirs(out_dir, exist_ok=True)

                cmd = f"libreoffice --headless --convert-to html --outdir '{out_dir}' '{file_path}'"
                returncode, stdout, stderr = await run_subprocess_with_timeout(cmd)

                if returncode != 0:
                    logger.error(f"Error LibreOffice: {stderr.decode()}")
                    # Fallback a extracción de texto simple si falla
                    if ext == ".docx": content = extract_text_from_docx(file_path)
                    elif ext == ".odt": content = extract_text_from_odt(file_path)
                else:
                    ...
                    if html_files:
                        ...
                        with open(html_path, "r", encoding="utf-8", errors="ignore") as f:
                            # Convertir HTML simple a texto/markdown muy básico o devolver HTML limpio
                            # Por simplicidad, devolveremos el texto extraído del HTML + referencias a imágenes
                            ...
                            if images:
                                return {
                                    "filename": download.suggested_filename,
                                    "type": f"{ext}_converted",
                                    "content_preview": content[:2000] + "..." if len(content)>2000 else content,
                                    "full_content": content,
                                    "extracted_images": images,
                                    "info": f"Documento convertido. Se encontraron {len(images)} imágenes incrustadas."
                                }
                    else:
                        content = "Error: No se generó archivo HTML."
            ...
            try:
                logger.info(f"Convirtiendo PDF {file_path} a imágenes con pdftoppm...")

                out_dir = os.path.join(save_dir, f"pdf_images_{uuid.uuid4()}")
                os.makedirs(out_dir, exist_ok=True)

                # Usar pdftoppm para convertir páginas a png
                # Sintaxis: pdftoppm -png input.pdf output_prefix
                cmd = f"pdftoppm -png '{file_path}' '{os.path.join(out_dir, 'page')}'"
                returncode, stdout, stderr = await run_subprocess_with_timeout(cmd)

                images = []
                # Buscar las imágenes generadas (page-1.png, page-01.png, etc)
                for root, _, files in os.walk(out_dir):
                    for file in files:
                        if file.endswith(".png"):
                            images.append(os.path.join(root, file))

                # Ordenar imágenes por número de página
                images.sort()

                # Intentar extraer texto también con pdftotext si es posible, para tener algo indexable
                text_content = ""
                try:
                    cmd_text = f"pdftotext '{file_path}' -"
                    ret, std_text, _ = await run_subprocess_with_timeout(cmd_text, timeout=30)
                    if ret == 0:
                        text_content = std_text.decode('utf-8', errors='ignore')
                    else:
                        text_content = "(No se pudo extraer texto seleccionable)"
                except:
                    text_content = "(No se pudo extraer texto seleccionable)"
                ...
                if images:
                    return {
                        ...
                        "extracted_images": images,
                        "info": f"PDF convertido a {len(images)} imágenes con pdftoppm."
                    }
                else:
                    # Fallback si falla pdftoppm
                    content = extract_text_from_pdf(file_path)
                    return {"filename": filename, "type": ".pdf_text", "full_content": content}

            except Exception as e:
                logger.error(f"Error pdftoppm: {e}")
                content = extract_text_from_pdf(file_path)
                return {"filename": filename, "type": ".pdf_text", "full_content": content}
